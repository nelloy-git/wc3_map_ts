...
export class w3iMapInfo extends Obj{
    ...
    toBinary(){
        ...
        let flags = 0
        if (this.hide_minimap_in_preview){flags = flags | 0x0001}
        if (this.modify_ally_properties){flags = flags | 0x0002}
        if (this.melee){flags = flags | 0x0004}
        if (this.mask_areas_are_partially_visible){flags = flags | 0x0010}
        if (this.fixed_player_settings_for_custom_forces){flags = flags | 0x0020}
        if (this.use_custom_forces){flags = flags | 0x0040}
        if (this.use_custom_techtree){flags = flags | 0x0080}
        if (this.use_custom_abilities){flags = flags | 0x0100}
        if (this.use_custom_upgrades){flags = flags | 0x0200}
        if (this.show_waves_on_cliff_shores){flags = flags | 0x0800}
        if (this.show_waves_on_rolling_shores){flags = flags | 0x1000}
        raw += int2byte(flags)

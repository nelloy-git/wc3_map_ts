...
export namespace Mouse {
    ...
    export function addAction(event: Event,
                              callback: (this:void, event: Event, pl:jplayer, btn:jmousebuttontype)=>void){
        return _actions.get(event)?.add(callback)
    }

    export function removeAction(action: Action<[Event, jplayer, jmousebuttontype], void>){
        let found = false
    ...
    let _actions = new Map<Event, ActionList<[Event, jplayer, jmousebuttontype]>>([
        ['DOWN', new ActionList()],
        ['MOVE', new ActionList()],
        ['UP', new ActionList()],
    ])
    ...
    let _triggers = IsGame() ? new Map<Event, jtrigger>([
        ['DOWN', CreateTrigger()],
        ['MOVE', CreateTrigger()],
        ['UP', CreateTrigger()]
    ]) : undefined
    ...
    function _runActions(event: Event){
        _setNext()
    ...
    if(IsGame()){
        ...
        TriggerAddAction(move_trig, ():void => {_runActions('MOVE')})
        TriggerAddAction(up_trig, ():void => {_runActions('UP')})

...
export class Frame extends Handle.Handle<jframehandle> {
    ...
    constructor(handle: jframehandle | Fdf.Fdf, is_simple?: boolean){
        ...
        this._is_tooltip = 0;
        this._level = 0;
        this._color = new Utils.Color(1, 1, 1, 1)
        this._enable = true;
        this._actions = new Map<Frame.Event, Utils.ActionList<[Frame, Frame.Event, jplayer]>>()
    }
    ...
    initEvents(events: Frame.Event[]){
        ...
        events.forEach(event => {
            ...
            this._actions.set(event, new Utils.ActionList())
        })

        this._trig_events = []
        for (let [wc_event, _] of Frame._wc2event){
            let trig_event = Handle.hTriggerEvent.newFrameEvent(this.handle, wc_event)
    ...
    get color(){return new Utils.Color(this._color)}
    set color(c: Utils.Color){
        this._color = new Utils.Color(c)

...
const SCALE_DMG = 'dmg'
const SCALE_CLOUD_DUR = 'cloudDur'
const SCALE_ACID_DUR = 'acidDur'
const SCALE_ACID_VAL = 'acidVal'
const SCALE_TICKS_PER_SEC = 'ticksPerSec'

// Extra
const ANIM_MODEL = ['model']

// Init
const ABIL_CACHED = AbilityJson.load(Cached,
    [SCALE_CAST_TIME, SCALE_DMG, SCALE_CLOUD_DUR, 
     SCALE_ACID_DUR, SCALE_ACID_VAL, SCALE_TICKS_PER_SEC],
    [ANIM_MODEL])
const TData = new AbilityData(ABIL_CACHED)
const Casting = new Abil.TCasting<[]>()
...
function clear(abil: Abil.IFace<[]>){
    let data = CastData.get(abil)
    if (data){
        data.detach()
        data.destroy()
...
export let VoodooPoison = new Abil.TAbility(Casting, TData, Abil.TTargetingNoTarget)